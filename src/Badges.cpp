...
List* getClassicList() {
    return ListRegistry::getListByID("classic"_spr);
}

List* getPlatformerList() {
    return ListRegistry::getListByID("platformer"_spr);
}

static bool isEditor(int accountID) {
    for (const auto& editor : getClassicList()->m_settings->staff->m_members) {
        if (editor->m_accountID == accountID) {
            return true;
        }
    }
    return false;
...
static std::optional<StaffRole> getRole(int accountID) {
    for (const auto& editor : getClassicList()->m_settings->staff->m_members) {
        if (editor->m_accountID == accountID) {
            return editor->m_role;
        }
    }
    return std::nullopt;
}
...
class $modify(MyProfilePage, ProfilePage) {
    void loadPageFromUserInfo(GJUserScore* a2) {
        ...
        if (isEditor(a2->m_accountID)) {
            ...
            if (role.has_value()) {
                switch (role.value()) {
                    case StaffRole::Owner: badgeString = "GDCP_owner.png"_spr; tag = 1; break;
                    case StaffRole::Admin: badgeString = "GDCP_coOwner.png"_spr; tag = 2; break;
                    case StaffRole::Dev: badgeString = "GDCP_developer.png"_spr; tag = 3; break;
                    case StaffRole::Helper: badgeString = "GDCP_moderator.png"_spr; tag = 4; break;
                    default: break;
                }
